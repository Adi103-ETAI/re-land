...
export default function Validation() {
  ...
  const hasMismatch = Math.abs(mismatch) > 0.001;
  const score = Math.max(70, Math.round(100 - (hasMismatch ? 6 : 0) - (currentCase.dupMatch ? 4 : 0) - 2));
  ...
  return (
    <div>
      ...
      <div className={`card mb-3 border-l-4 ${hasMismatch ? "border-l-[#B5651D]" : "border-l-[#496D21]"}`}>
        ...
        <div className="text-xs text-[var(--gray-600)]">{hasMismatch ? `Difference: ${Math.abs(mismatch).toFixed(2)} Hectare — flagged for officer review.` : "Document and database areas match exactly."}</div>
        <div className="flex gap-2 mt-3 flex-wrap"><button className="btn btn-teal btn-sm">Accept AI Suggestion</button><button className="btn btn-ghost btn-sm">Edit Manually</button><Link href="/verification" className="btn btn-ghost btn-sm">Send for Verification</Link></div>
      </div>

      <div className={`card mb-3 border-l-4 ${currentCase.dupMatch ? "border-l-[#A13A2C]" : "border-l-[#496D21]"}`}>
        <div className="flex justify-between items-center mb-2"><b className="text-sm">Survey number conflict</b><span className={`text-[11px] font-extrabold px-2 py-1 rounded-md uppercase ${currentCase.dupMatch ? "bg-[#FDF3F0] text-[#A13A2C]" : "bg-[#F0F7EC] text-[#496D21]"}`}>{currentCase.dupMatch ? "HIGH" : "RESOLVED"}</span></div>
        ...
        <div className="flex gap-2 mt-3"><button className="btn btn-ghost btn-sm">Compare records</button><button className="btn btn-teal btn-sm">Not a duplicate</button></div>
      </div>
