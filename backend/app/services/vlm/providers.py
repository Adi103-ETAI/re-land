...
For each field return {"value": string|null, "confidence": 0-1, "bbox": [ymin,xmin,ymax,xmax] normalized 0-1000 or null}. CRITICAL bbox order is Y-X-Y-X: ymin=top, xmin=left, ymax=bottom, xmax=right. Example: if Village "खेड" is at top-center, bbox might be [90,320,130,380] (ymin=90, xmin=320). Another: Survey "125/2" in middle table at [355,215,385,280]. Bbox must tightly bound the VALUE text only (not the label). Width must be > height for horizontal text (e.g. w ~60, h ~30). Village bbox around भरे/Bhare after गाव:, tehsil bbox around मुळशी after तालुका:, district bbox around पुणे after जिल्हा:, surveyNo bbox around 278 number, khataNo bbox around KH-89, ownerName bbox around the full owner block on right, area bbox around the hectare number in left table (e.g. २.३६३५).
