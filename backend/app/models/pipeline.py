...
from sqlalchemy import Column, String, Integer, DateTime, Float, Boolean, ForeignKey, Enum as SAEnum, JSON
...
class ProcessingStage(Base, BaseRecord):
    ...
    __table_args__ = (
        # One active attempt per stage at a time
...
class ProcessingAttempt(Base, BaseRecord):
    ...
    config_used = Column(JSON, nullable=True)  # Model/version/config used
    ...
    failure_category = Column(String, nullable=True)  # timeout, api_error, validation_error, etc.
